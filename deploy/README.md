...
- `web.config` — server-owned; carries `maxAllowedContentLength`, **62914560 while
  `FileStorage:MaxFileSizeBytes` is 52428800** — the two move together, and the number is
  deliberately **above** the application's own limit rather than equal to it. Because this file
  is never overwritten, a node built before that number was raised still holds the old `52428800`
  — now *below* the application's limit, so a full-size upload there is refused by IIS with a bare
  `404.13`. A node whose `web.config` was regenerated by a clean publish is worse off still: the
  published file carries no `<requestLimits>` at all, so IIS falls back to its own 30,000,000
  default. Re-apply §6.5 of the production deployment guide on every existing node once, and check
  for the element rather than assuming a value is there. IIS measures
  Content-Length — which carries the multipart envelope as well as the file — and it answers before
  ASP.NET Core runs, so a value set to the file size would reject every oversized upload itself
  with a bare `404.13` that the SPA cannot turn into a message. With headroom the application
  answers instead, with the size limit in the `detail` field, and IIS remains the outer guard. The
  matching in-process limit is raised in code, per upload endpoint; web.config cannot reach it.
...
consumer keeps its class name but starts consuming a different message type, its queue stays bound to
the old exchange forever.

`AppraisalCompletedWebhookConsumer` now consumes `AppraisalResultReadyIntegrationEvent` instead of
`AppraisalCompletedIntegrationEvent` (the outbound `APPRAISAL_COMPLETED` webhook waits for the
post-approval summary to be attached). Its queue name is unchanged — `appraisal-completed-webhook` —
so without an unbind:

- every completed appraisal still routes a copy of `AppraisalCompletedIntegrationEvent` into that
  queue, finds no consumer for the type, and lands in `appraisal-completed-webhook_skipped`, which
  grows with no error anywhere;
- anything already sitting unconsumed in the queue at cutover is skipped rather than delivered, so
  **those appraisals get no webhook at all** — not from the old path, not from the new one.

Each app node has its own broker, so do this **per node**:

1. Stop **both** app nodes before starting either new one. Do not roll one at a time: a node on the
   old build fires the webhook off the completion event while a node on the new build fires it off
   the result-ready event, and LOS receives `APPRAISAL_COMPLETED` twice.
2. Confirm `appraisal-completed-webhook` is drained (management UI, or `rabbitmqctl list_queues`).
3. Remove the binding from the `Shared.Messaging.Events:AppraisalCompletedIntegrationEvent` exchange
   to that queue — or simply delete the queue and let the app recreate it on start.
4. Start the nodes. Verify the queue's bindings now list only the `AppraisalResultReadyIntegrationEvent`
   exchange, and that `..._skipped` stops growing.

After cutover, check for appraisals completed during the window that never got a webhook
(`integration.WebhookDeliveries` has no `APPRAISAL_COMPLETED` row for them) and re-notify with
...
websites, the OAuth2 certificates with private-key ACLs, and the upload limit (62914560 bytes —
see above).
