...
namespace Assignment.Tests.Workflow.Performance;
...
public class WorkflowPerformanceTests
{
    ...
    public async Task ActivityExecution_ConcurrentActivities_MaintainsPerformance()
    {
        ...
        for (int i = 0; i < concurrentActivities; i++)
        {
            var context = new ActivityContext
            {
    ...
    public async Task WorkflowMemoryUsage_LargeVariableSets_StaysWithinLimits()
    {
        ...
        for (int i = 0; i < workflowCount; i++)
        {
            ...
            {
                largeVariables[$"var_{i}_{j}"] = $"Large data string for variable {i}-{j} with substantial content to simulate real-world variable storage requirements";
            }
