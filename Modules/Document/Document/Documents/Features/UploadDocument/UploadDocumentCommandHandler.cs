...
) : ICommandHandler<UploadDocumentCommand, UploadDocumentResult>
{
    ...
    public async Task<UploadDocumentResult> Handle(UploadDocumentCommand command, CancellationToken cancellationToken)
    {
        ...
        var session = await _uploadSessionRepository.GetByIdForUpdateAsync(command.UploadSessionId, cancellationToken);
        if (session == null)
        {
        ...
        var duplicateExists = await _documentRepository.ExistsAsync(d => d.Checksum == checksum, cancellationToken);
        if (duplicateExists)
        {
            logger.LogWarning(
