...
public record AppraisalForCollateralResult(
    ...
    string? CompanyId,
    string? CompanyName,
    // HostCompanyCode from auth.Companies — 4-char external valuer code for the outbound Collateral Result interface.
    string? CompanyCode,
    ...
    string? CustomerName,
    // --- Engagement-enrichment fields (for the outbound Collateral Result interface) ---
    // Appraisal-level forced-sale value from ValuationAnalyses.ForcedSaleValue (matches AppraisedValue source).
    decimal? ForcedSaleValue,
    // Bank-side internal valuer display name, denormalized on the latest assignment
    // (AppraisalAssignment.InternalAppraiserName). Null when not captured.
...
public record ProjectForCollateral(
    string ProjectType,          // ProjectType code: "U" (Condo) | "LB" (LandAndBuilding) | "L" (Land)
    string? ProjectName,
    string? Developer,
...
public record MachineryIdentityForCollateral(
    ...
    // Useful-life years from the selected MachineryCost method (MachineCostItem.LifeSpanYears).
    // Null when no machinery cost item / life span was entered. Used by the outbound interface (Life Year).
    decimal? LifeYear
);
