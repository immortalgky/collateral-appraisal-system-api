...
public class ProjectTowerImageConfiguration : IEntityTypeConfiguration<ProjectTowerImage>
{
    public void Configure(EntityTypeBuilder<ProjectTowerImage> builder)
    {
        builder.ToTable("ProjectTowerImages");

        builder.HasKey(i => i.Id);
        builder.Property(i => i.Id).HasDefaultValueSql("NEWSEQUENTIALID()");

        builder.Property(i => i.ProjectTowerId).IsRequired();
        ...
        // Composite index covers lookups by ProjectTowerId; single-column index is redundant.
        builder.HasIndex(i => i.GalleryPhotoId);
        builder.HasIndex(i => new { i.ProjectTowerId, i.DisplaySequence });
