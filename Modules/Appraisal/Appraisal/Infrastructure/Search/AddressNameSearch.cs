    ...
    : IAddressNameSearch
{
    ...
    public async Task<AddressNameMatch> MatchAsync(string? term, CancellationToken cancellationToken = default)
    {
        ...
        var row = await connection.QuerySingleAsync<ProbeRow>(new CommandDefinition(
            // The ADO default, matching QuickSearchQueryHandler — this probe runs first on the same
            // per-request connection, so the two have to agree or the tighter one is meaningless.
            // Healthy cost is ~5 ms; the cancellation token is what ends it early when the user
            // types on.
