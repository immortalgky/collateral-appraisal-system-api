...
/// <summary>Handler for getting project land details.</summary>
public class GetProjectLandQueryHandler(
    IProjectRepository projectRepository
) : IQueryHandler<GetProjectLandQuery, GetProjectLandResult>
{
    public async Task<GetProjectLandResult> Handle(
        GetProjectLandQuery query,
        CancellationToken cancellationToken)
    {
        var project = await projectRepository.GetWithFullGraphAsync(query.AppraisalId, cancellationToken);

        if (project is null || project.Land is null)
            return null!;

        var land = project.Land;

        var titles = land.Titles.Select(t => new ProjectLandTitleResultDto(
            Id: t.Id,
        ...
        return new GetProjectLandResult(
            Id: land.Id,
