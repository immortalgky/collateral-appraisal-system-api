...
/// <summary>
/// Static parser for the project-units Excel upload format.
/// Shared between <see cref="UploadProjectUnitsCommandHandler"/> and the block-reappraisal
/// re-match handler so Excel parsing is never duplicated.
/// </summary>
public static class ProjectUnitExcelParser
{
    /// <summary>
    /// Parses a Condo units Excel stream. Row 1 is headers; data starts at row 2.
    /// Headers are resolved case- and whitespace-insensitively.
    /// Rows where both Floor and RoomNumber are blank are skipped.
    /// </summary>
    public static List<ProjectUnit> ParseCondoExcel(Stream stream, Guid projectId)
    {
    ...
    /// <summary>
    /// Parses a LandAndBuilding units Excel stream. Row 1 is headers; data starts at row 2.
    /// Rows where both PlotNumber and HouseNumber are blank are skipped.
    /// </summary>
    public static List<ProjectUnit> ParseLandAndBuildingExcel(Stream stream, Guid projectId)
    {
        using var workbook = new XLWorkbook(stream);
        var worksheet = workbook.Worksheets.First();
        var headers = BuildHeaderMap(worksheet);
    ...
    /// Throws <see cref="BadRequestException"/> with the canonical (first) alias name if none match.
    /// </summary>
    public static int Resolve(Dictionary<string, int> headers, params string[] aliases)
    {
