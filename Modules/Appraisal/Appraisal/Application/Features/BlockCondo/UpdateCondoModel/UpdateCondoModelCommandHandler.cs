...
/// <summary>
/// Handler for updating a condo model
/// </summary>
public class UpdateCondoModelCommandHandler(
    IAppraisalUnitOfWork unitOfWork,
    IAppraisalRepository appraisalRepository
) : ICommandHandler<UpdateCondoModelCommand>
{
    public async Task<Unit> Handle(
        UpdateCondoModelCommand command,
        CancellationToken cancellationToken)
    {
        ...
        // 2. Find the model in the collection
        var model = appraisal.CondoModels.FirstOrDefault(m => m.Id == command.ModelId)
                    ?? throw new InvalidOperationException($"Condo model {command.ModelId} not found");
