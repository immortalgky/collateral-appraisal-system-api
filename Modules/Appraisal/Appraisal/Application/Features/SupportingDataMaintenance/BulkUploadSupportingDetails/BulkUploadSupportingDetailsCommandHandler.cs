...
) : ICommandHandler<BulkUploadSupportingDetailsCommand, BulkUploadSupportingDetailsResult>
{
    ...
    {
        // ── 1. Role guard (same rule as single-row create) ─────────────────
        if (currentUserService.IsInRole("IntAppraisalChecker") ||
            currentUserService.IsInRole("ExtAppraisalChecker"))
        {
            throw new UnauthorizedAccessException(
                "Checkers are not allowed to upload supporting details.");
        }
