...
public class GetComparativeAnalysisTemplatesEndpoint : ICarterModule
{
    public void AddRoutes(IEndpointRouteBuilder app)
    {
        app.MapGet("/comparative-analysis-templates",
                ...
                {
                    var query = new GetComparativeAnalysisTemplatesQuery(activeOnly ?? false);
                    var result = await sender.Send(query);
                    return Results.Ok(new GetComparativeAnalysisTemplatesResponse(result.Templates));
                })
            .WithName("GetComparativeAnalysisTemplates")
            .Produces<GetComparativeAnalysisTemplatesResponse>()
