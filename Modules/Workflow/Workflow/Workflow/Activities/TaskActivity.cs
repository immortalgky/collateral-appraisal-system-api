...
public class TaskActivity : WorkflowActivityBase
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var initialAssignmentStrategies =
                GetProperty<List<string>>(context, "initialAssignmentStrategies", ["RoundRobin"]);
            var revisitAssignmentStrategies =
                GetProperty<List<string>>(context, "revisitAssignmentStrategies", ["RoundRobin"]);
            ...
            // Step 1: Try custom assignment service first (the highest priority)
            ...
            if (!string.IsNullOrEmpty(customServiceName))
            {
                ...
                if (customAssignmentResult != null) return customAssignmentResult;
            }

            // Step 2: Try runtime overrides second (API-provided overrides)
            if (context.RuntimeOverrides is { HasAssignmentOverride: true })
            {
                var runtimeOverrideResult = await ProcessRuntimeOverrideAsync(context, cancellationToken);
                if (runtimeOverrideResult != null) return runtimeOverrideResult;
            }

            // Step 3: Consider external configuration third (if available)
            // TODO: To check whether is there a external configuration for this activity

            // Default systemed assignment strategies
            // Step 4: Try to find the previous handler first (simplified logic)
            ...
            var previousOwnerResult =
                await previousOwnerSelector.SelectAssigneeAsync(previousOwnerContext, cancellationToken);

            if (previousOwnerResult.IsSuccess && !string.IsNullOrEmpty(previousOwnerResult.AssigneeId))
            {
                _logger.LogDebug("Found previous handler for activity {ActivityId}: {AssigneeId}",
                    context.ActivityId, previousOwnerResult.AssigneeId);

                // Assign to the previous handler and return early
                ...
                var previousOwnerOutput = new Dictionary<string, object>
                {
                    ...
                    ["assignmentStrategy"] = nameof(AssigneeSelectionStrategy.PreviousOwner),
                    ...
                    [$"{NormalizeActivityId(context.ActivityId)}_assignmentMetadata"] =
                        previousOwnerResult.Metadata ?? new Dictionary<string, object>()
                };

                return ActivityResult.Pending(previousOwnerOutput);
            }

            // Step 5: No previous handler found, use assignment strategies
            _logger.LogDebug("No previous handler found for activity {ActivityId}, using assignment strategies",
                context.ActivityId);
            ...
            else
            {
                ...
                strategiesToUse = initialAssignmentStrategies;
            ...
            {
                _logger.LogWarning(
                    "Assignment strategies failed for activity {ActivityId}. Attempting admin pool fallback",
                    context.ActivityId);
            ...
            if (result.IsSuccess)
            {
                ...
                // Log assignment change to audit service (same transaction)
                await _auditService.LogAssignmentChangeAsync(
                    context,
                    null, // no previous assignee
                    result.AssigneeId,
            ...
            // TODO: Review later
            // Execute onComplete actions after a successful assignment
        ...
        catch (Exception ex)
        {
            ...
            {
                _logger.LogError(actionEx, "Error executing onError actions for activity {ActivityId}",
                    context.ActivityId);
            }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        catch (Exception ex)
        {
            ...
            // Execute onError actions when the task résumé fails
            try
            {
                await ExecuteLifecycleActionsAsync(context, ActivityLifecycleEvent.OnError, cancellationToken);
            }
        ...
        AssignmentContext assignmentContext)
    {
        try
        {
            if (!string.IsNullOrEmpty(config.AdminPoolId))
            {
                ...
                _logger.LogInformation(
                    "Attempting admin pool fallback for activity {ActivityId}. Admin Pool: {AdminPoolId}",
                    assignmentContext.ActivityName, config.AdminPoolId);

                return Task.FromResult(AssigneeSelectionResult.Success(config.AdminPoolId,
        ...
        {
            _logger.LogError(ex, "Admin pool fallback failed for activity {ActivityId}",
                assignmentContext.ActivityName);
        ...
        ActivityContext context)
    {
        try
        {
            ...
            foreach (var kvp in decisionConditions)
            {
                ...
                catch (Exception conditionEx)
                {
                    _logger.LogWarning(conditionEx,
                        "Failed to evaluate decision condition '{Condition}' for TaskActivity {ActivityId}. Skipping condition.",
            ...
            _logger.LogDebug("No decision conditions matched for TaskActivity {ActivityId}. Using fallback decision.",
                context.ActivityId);
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            // Check if custom service wants to handle the assignment
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(customResult.SpecificAssignee))
            {
                ...
                // Create activity execution record and log assignment atomically
                await _persistenceService.ExecuteInTransactionAsync(async () =>
                {
                    await _persistenceService.CreateActivityExecutionAsync(
                        context.WorkflowInstance.Id,
                        context.ActivityId,
                        "Task Activity",
                        ActivityType,
            ...
            if (customResult.CustomStrategies != null && customResult.CustomStrategies.Any())
            {
                ...
                // Add custom properties to the assignment context
                ...
                var strategyResult = await _persistenceService.ExecuteInTransactionAsync(
                    () => ExecuteAssignmentWithLockAsync(assignmentContext, cancellationToken),
                ...
                if (strategyResult.IsSuccess)
                {
                    ...
                    await _persistenceService.ExecuteInTransactionAsync(async () =>
                    {
                        ...
                        await _auditService.LogAssignmentChangeAsync(
                            ...
                            strategyResult.AssigneeId,
                            customResult.Reason ??
                            $"Custom assignment using strategies: {string.Join(", ", customResult.CustomStrategies)}",
                            AssignmentChangeType.InitialAssignment,
                    ...
                    _logger.LogInformation(
                        "Custom assignment completed using strategies {Strategies} - assigned to: {Assignee}",
                ...
                else
                {
                    ...
                    var failureOutput = CreateAssignmentOutput(
                        ...
                        customResult.Metadata,
                        $"Custom assignment failed: {strategyResult.ErrorMessage ?? "Unknown error"}", false);
                    ...
                    return ActivityResult.Failed($"Custom assignment failed: {strategyResult.ErrorMessage}",
                        failureOutput);
                }
            }
            ...
            _logger.LogWarning(
                "Custom assignment service indicated it should handle assignment but provided no specific instructions");
            return ActivityResult.Failed("Custom assignment service error: no assignment instructions provided");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error processing custom assignment result for activity {ActivityId}",
                context.ActivityId);
            throw;
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            _logger.LogInformation(
                "Processing runtime assignment override for activity {ActivityId}. Override by: {OverrideBy}, Reason: {Reason}",
            ...
            {
                // Validate assignee has a required role
                var validationResult =
                    await ValidateAssigneeRole(runtimeOverride.RuntimeAssignee, assigneeRole, cancellationToken);
                ...
                await _persistenceService.ExecuteInTransactionAsync(async () =>
                {
                    ...
                    await _auditService.LogAssignmentChangeAsync(
                        ...
                        null,
                        runtimeOverride.RuntimeAssignee,
                        runtimeOverride.OverrideReason ?? "Runtime assignment override",
                        AssignmentChangeType.RuntimeOverride,
            ...
            if (runtimeOverride.RuntimeAssignmentStrategies != null &&
                runtimeOverride.RuntimeAssignmentStrategies.Any())
            {
                var assignmentContext = new AssignmentContext
                {
                    ActivityName = activityName,
                ...
                // Add runtime override properties to the assignment context
                ...
                if (strategyResult.IsSuccess)
                {
                    ...
                    await _persistenceService.ExecuteInTransactionAsync(async () =>
                    {
                        ...
                        await _auditService.LogAssignmentChangeAsync(
                            ...
                            null,
                            strategyResult.AssigneeId,
                            runtimeOverride.OverrideReason ?? "Runtime strategy override",
                            AssignmentChangeType.RuntimeOverride,
                            metadata,
                            cancellationToken);
                    }, IsolationLevel.ReadCommitted, cancellationToken);
                    ...
                    _logger.LogInformation(
                        "Runtime override completed using strategies {Strategies} - assigned to: {Assignee}",
                ...
                else
                {
                    ...
                    return ActivityResult.Failed(
                        $"Runtime assignment override failed: {strategyResult.ErrorMessage ?? "Unknown error"}");
                }
            }

            // If we get here, a runtime override exists but has no valid assignment instructions
        ...
        {
            _logger.LogError(ex, "Error processing runtime assignment override for activity {ActivityId}",
                context.ActivityId);
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (string.IsNullOrEmpty(requiredRole)) return new AssigneeValidationResult { IsValid = true };
        ...
        {
            _logger.LogError(ex, "Error validating assignee {AssigneeId} for role {RequiredRole}", assigneeId,
                requiredRole);
            return new AssigneeValidationResult
            {
                IsValid = false,
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (executionResult.IsSuccess)
                _logger.LogInformation(
                    "Successfully executed {SuccessCount}/{TotalCount} {LifecycleEvent} actions for activity {ActivityId}",
                    executionResult.SuccessfulActions,
                    executionResult.SuccessfulActions + executionResult.FailedActions + executionResult.SkippedActions,
                    lifecycleEvent, context.ActivityId);
            else
                _logger.LogWarning(
                    "Some {LifecycleEvent} actions failed for activity {ActivityId}: {FailedCount}/{TotalCount} failed. Errors: {Errors}",
                    lifecycleEvent, context.ActivityId, executionResult.FailedActions,
                    executionResult.SuccessfulActions + executionResult.FailedActions + executionResult.SkippedActions,
        ...
        try
        {
            ...
            foreach (var propertyName in alternativePropertyNames)
            {
                ...
                if (alternativeActions != null && alternativeActions.Any())
                {
                    ...
                    _logger.LogDebug(
                        "Found {ActionCount} actions in alternative property '{PropertyName}' for {LifecycleEvent}",
    ...
    /// <param name="requiredRole">Required role</param>
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>True if user has a required role</returns>
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            // If a role is not in known patterns, assume it valid (fail-open for unknown roles)
            _logger.LogWarning(
                "Unknown role validation requested: {RequiredRole} for user {AssigneeId}. Allowing access.",
    ...
    /// <summary>
    /// Extracts a user role from user ID (basic implementation based on email patterns)
    ...
    private static string ExtractUserRoleFromId(string assigneeId)
    {
        ...
        // Default to a basic user role
        return "user";
    }

    /// <summary>
    /// Executes assignment with explicit sp_getapplock to prevent race conditions
    /// </summary>
    private async Task<AssigneeSelectionResult> ExecuteAssignmentWithLockAsync(
        AssignmentContext assignmentContext,
        CancellationToken cancellationToken)
    {
        // Create a lock resource name for this specific assignment context
        var lockResource = GenerateLockResource(assignmentContext);

        _logger.LogDebug("Acquiring assignment lock for resource: {LockResource}", lockResource);

        try
        {
            // Acquire exclusive lock using sp_getapplock
            await AcquireAssignmentLockAsync(lockResource, cancellationToken);

            // Execute the assignment strategy within the lock
            var result = await _cascadingEngine.ExecuteAsync(assignmentContext, cancellationToken);

            _logger.LogDebug("Assignment completed with lock {LockResource}. Success: {IsSuccess}",
                lockResource, result.IsSuccess);

            return result;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Assignment failed with lock {LockResource}", lockResource);
            throw;
        }
        // Lock will be automatically released when transaction commits/rollbacks
    }

    /// <summary>
    /// Generates a unique lock resource name for assignment context
    /// </summary>
    private static string GenerateLockResource(AssignmentContext context)
    {
        // Create a lock resource based on activity and groups to ensure granular locking
        var activityName = context.ActivityName;
        var groupIdentifier = context.UserGroups.Any()
            ? string.Join("_", context.UserGroups.OrderBy(g => g))
            : "NoGroup";

        // Sanitize the resource name (SQL Server app lock names have limits)
        var resource = $"Assignment_{activityName}_{groupIdentifier}";

        // Truncate if too long (SQL Server limit is 255 chars)
        if (resource.Length > 200)
        {
            var hash = resource.GetHashCode().ToString("X");
            resource = $"Assignment_{activityName}_{hash}";
        }
    ...
    /// <summary>
    /// Acquires exclusive assignment lock using sp_getapplock with proper return code handling
    /// </summary>
    private async Task AcquireAssignmentLockAsync(string lockResource, CancellationToken cancellationToken)
    {
        try
        {
            var lockResult = await _persistenceService.AcquireApplicationLockAsync(
                lockResource, "Exclusive", 30000, cancellationToken);

            // Handle sp_getapplock return codes
            switch (lockResult)
            {
                case 0:
                    _logger.LogDebug("Successfully acquired assignment lock immediately: {LockResource}", lockResource);
                    break;
                case 1:
                    _logger.LogDebug("Successfully acquired assignment lock after waiting: {LockResource}",
                        lockResource);
                    break;
                case -1:
                    throw new TimeoutException($"Assignment lock acquisition timed out for {lockResource}");
                case -2:
                    throw new OperationCanceledException(
                        $"Assignment lock acquisition was canceled for {lockResource}");
                case -3:
                    throw new InvalidOperationException(
                        $"Assignment lock acquisition deadlock victim for {lockResource}");
                default:
                    throw new InvalidOperationException(
                        $"Unexpected sp_getapplock result {lockResult} for {lockResource}");
            }
        }
        catch (Exception ex) when (!(ex is TimeoutException or OperationCanceledException))
        {
            _logger.LogError(ex, "Failed to acquire assignment lock: {LockResource}", lockResource);
            throw new InvalidOperationException($"Could not acquire assignment lock for {lockResource}", ex);
        }
    }
