...
{
    private readonly IWorkflowPersistenceService _persistenceService = persistenceService;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!result.IsSuccess) throw new InvalidOperationException($"Failed to select assignee: {result.ErrorMessage}");
