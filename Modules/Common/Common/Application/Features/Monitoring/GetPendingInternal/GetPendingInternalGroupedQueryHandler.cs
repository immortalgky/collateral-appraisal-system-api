...
/// <summary>
/// Returns grouped counts for the Pending Internal tab.
///
/// Design choice (option b — separate route): the grouped endpoint is distinct from the list
/// endpoint so the list response contract is not changed and the frontend TypeScript types
/// remain stable. The grouped result is always ordered by Count DESC and capped at 200 rows
/// (PIC / Activity cardinality is low in practice).
///
/// Supported groupBy values: "pic", "company", "activity".
/// </summary>
public class GetPendingInternalGroupedQueryHandler(
    ISqlConnectionFactory connectionFactory,
    MonitoringScopeService scopeService)
    : IQueryHandler<GetPendingInternalGroupedQuery, MonitoringGroupedResult>
{
...
{where}";
    ...
    /// <summary>
    /// Maps the groupBy discriminator to the SQL key/label/groupBy column expressions.
    /// PIC uses COALESCE(NULLIF(PIC, ''), AssignedTo) per Assumption A3.
    ...
    private static (string keyExpr, string labelExpr, string groupByExpr) ResolveGrouping(string groupBy) =>
        groupBy.ToLowerInvariant() switch
        {
            ...
            // Label = MAX(PIC) shows the display name; both 1:1 within a group so MAX is just an aggregate hack.
            "pic" => (
                "AssignedTo",
                "MAX(PIC)",
                "AssignedTo"),
