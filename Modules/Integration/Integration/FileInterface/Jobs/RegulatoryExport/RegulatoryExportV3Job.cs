...
/// respect except where the rows come from: <see cref="IRegulatoryExportV3Query"/> reads the appraisal
...
/// </summary>
public class RegulatoryExportV3Job(
    IRegulatoryExportV3Query query,
    ...
    ILogger<RegulatoryExportV3Job> logger)
{
    private const string JobTag = "[REGULATORY-EXPORT-V3]";
    ...
    public async Task ExecuteAsync(CancellationToken ct = default)
    {
        ...
        var cfg = await configProvider.GetAsync(FileInterfaceCodes.RegulatoryV3, ct);
        if (cfg is null || !cfg.IsActive)
        {
            logger.LogWarning(
                "{Tag} No active config row for '{Code}'; skipping", JobTag, FileInterfaceCodes.RegulatoryV3);
            return;
        }
        ...
        var prefix = cfg.FileNamePrefix ?? "REGULATORY_V3_";
